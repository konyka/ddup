...
- RESP 错误消息使用编译期 `sizeof` 长度，避免错误帧截断/越界；该检查不引入
  运行时扫描或额外分配，热路径成本为零。
- io_uring 状态查询与测试故障注入仅增加入口 NULL 判断；正常 ring 操作不增加
  锁竞争、分配或系统调用。
- TLS PAL 的无效输入保护只在 API 入口执行；已建立连接的握手、读写热路径不
  增加额外分支以外的分配或锁开销。
- 安全随机源仅增加一次 `buf == NULL && len != 0` 判断；正常 `ACL GENPASS`
  随机字节生成不增加分配或额外系统调用。
- 文件 I/O 参数检查位于 PAL 入口；有效持久化读写路径不增加额外分配，系统调用
  次数和数据复制保持不变。
- 文件路径与句柄边界测试仅运行于错误输入；正常 AOF/快照/Tier 访问不增加
  热路径分支之外的开销。
...
- TLS 客户端端点绑定在握手初始化时调用一次 OpenSSL 主机名检查；不会增加
  已建立连接的读写热路径分支、分配或系统调用，复制/总线重连失败仅发生在
  证书或名称不匹配的冷错误路径。
- Cluster bus 发布快速路径以固定头部读取和 O(1) 整数比较校验 magic、内嵌
  总长度及 payload 边界；合法帧保留原有零分配本地投递，不增加订阅者遍历成本。
- 集群 migrate/failover/pfail 的节点分配检查仅位于测试夹具，消除 LTO 的
  假定 NULL 解引用警告；不改变生产二进制热路径、分配行为或基准结果。
- Cluster bus 协议 setter 仅增加一次枚举比较；合法配置初始化和总线帧热路径
...
- MIGRATE deadline 饱和是在连接前的一次 64 位比较；正常 timeout 的发送/读取
  循环不增加系统调用或分配，只有不可表示的极大超时走 `UINT64_MAX` 冷边界。
- Cluster bus 端口转换只增加一次 32 位范围比较；合法监听和 MEET 连接不增加
  分配或系统调用，超过 16 位范围的配置在初始化/控制面冷路径中直接拒绝。
- Cluster announce setter 仅在控制面更新时同步 myself node 的固定地址字段；
...
- MIGRATE 的零 key 请求在连接和内存分配前直接返回；通常多 key 迁移的 idx
  分配、序列化及 pipeline 路径保持不变。
- 复制控制 API 仅在公共入口增加 NULL 判断；正常 PSYNC/backlog 数据路径不增加
  分配、锁或额外系统调用。
- server 配置入口保护仅作用于无效调用；有效 TLS/AOF/快照/运行周期路径保持
  原有分配、锁和系统调用行为。
- 连接迁移/输出 API 的安全检查位于控制面入口；有效连接的路由、缓冲和发送
  路径不增加分配或系统调用。
- 基础查询和数据库选择仅增加入口边界判断；有效查询仍为 O(1)，无分配和锁。
- 事件循环 NULL 检查仅作用于无效调用；有效 `server_run_once` 的过期扫描、
  路由和 I/O 调度流程不增加额外分配或系统调用。
- 集群 setter 与 nodes.conf 测试入口的 NULL 判断仅影响错误调用；正常 gossip、
  TLS 和配置更新路径不增加分配或系统调用。
- 节点计数一致性检查在集群控制入口执行；正常节点遍历仍为原有 O(N) 且不增加
  分配，避免热路径逐节点重复校验。
- 故障报告与状态恢复只在控制面入口检查有界计数；正常 gossip/故障仲裁保持
  原有固定数组操作和无分配行为。
- 状态恢复的逐节点校验仅发生在低频快照/worker 同步路径；正常命令和 gossip
  热路径不增加遍历成本。
- 总线帧入口仅增加一次节点计数判断；有效 gossip/FAIL/PONG 处理不增加复制或
  分配开销。
