...
> 对照：ddup `src/core/command.c` `CMD_TABLE`（202 个顶层命令）。
...
missing_top: acl blmove blmpop blpop brpop brpoplpush bzmpop bzpopmax bzpopmin debug eval_ro evalsha_ro failover fcall fcall_ro function latency lolwut module monitor replconf restore-asking sentinel wait waitaof xack xadd xautoclaim xclaim xdel xgroup xinfo xlen xpending xrange xread xreadgroup xrevrange xsetid xtrim
