...
- mt 模式 `KEYS/RANDOMKEY/SCAN` 已通过 worker 路由或聚合；`PUBSUB CHANNELS/NUMSUB/NUMPAT` 广播归并为全局视图；`MIGRATE` 按源 key hash 路由到 source worker，连接亲和迁移后复用有界网络迁移逻辑（IOCP/io_uring-op 不可安全迁移时返回明确错误）；`SHUTDOWN` 已由 home worker 执行并协调停止全部 worker；`PSUBSCRIBE/PUNSUBSCRIBE` 已通过 worker-local pattern registry 和 `pmessage` fan-out 支持；`ASKING` 已在每个 worker 的本地会话安全门中支持一次性标志；`SYNC/PSYNC/REPLICAOF/SLAVEOF/CLUSTER` 已由 worker 0 控制面适配（`mt_is_blocked` 当前清单仅保留阻塞 pop 族）。
