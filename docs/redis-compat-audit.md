...
> 对照：ddup `src/core/command.c` `CMD_TABLE`（181 个顶层命令）。
...
`ACL`, `BGREWRITEAOF`, `BGSAVE`, `COMMAND`, `DEBUG`, `FAILOVER`, `LATENCY`, `LOLWUT`, `MEMORY`, `MODULE`, `MONITOR`, `REPLCONF`, `RESTORE-ASKING`, `SLOWLOG`
...
- 复制/运维族：无 `REPLCONF`、`BGSAVE`、`BGREWRITEAOF`、`MONITOR`、`WAIT/WAITAOF`、`RESTORE-ASKING`（`REPLICAOF`/`ROLE`/`SLAVEOF` 已实现）。
