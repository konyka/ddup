...
- [x] **Phase 7+ — 长期**（子项全部完成）
  ...
  - [x] io_uring op 进阶：multishot recv + 256×64KB provided-buffer
    ...
    send buffers/SEND_ZC 早期评估后暂不启用（后续 Phase 97 已实现并保留显式环境门控）；bench 常驻 repost
  ...
  - [x] FUNCTION 库名错误边界：重复库名诊断限制输入长度并校验实际写出长度，
    防止超长 `#!lua name=` 元数据造成固定缓冲区越界（Phase 222）
  - [x] CONFIG 错误边界：未知配置参数错误使用有界渲染并检查截断结果，避免
    恶意超长参数导致 RESP 长度越界（Phase 223）
  - [x] Hash 字段 TTL 错误边界：`HEXPIRE/HPEXPIRE` 族未知选项限制渲染长度，
    防止恶意长参数造成错误响应越界（Phase 224）
  - [x] 管理参数错误边界补全：`ACL DRYRUN` 超长用户名错误安全截断并检查
    实际写出长度，避免用户输入造成栈缓冲区越界（Phase 225）
  - [x] LATENCY 诊断错误边界：`GRAPH/HISTORY` 超长事件名限制渲染长度并
    检查截断结果，避免诊断错误响应越界（Phase 226）
  - [x] Hash 字段通用选项边界：`HSETEX` 解析器对未知选项限制渲染长度并
    检查截断结果，覆盖字段写入路径的固定缓冲区安全（Phase 227）
  - [x] Pub/Sub 上下文错误边界：订阅状态下禁止命令的错误响应检查截断
    长度，避免固定缓冲区长度误用（Phase 228）
  - [x] INFO/ACL 元数据渲染安全：INFO 多段输出使用有界追加器，ACL 规则行
    最大规则集回归验证不会超出固定缓冲区（Phase 229）
  - [x] ACL 规则行追加安全：规则、密码和 channel 元数据统一经有界追加器
    拼接，容量耗尽时保留完整 `CRLF` 尾部（Phase 230）
  - [x] MONITOR 输出追加安全：quoted 参数长度溢出检查、预留失败传播与
    半条消息回滚，避免内存/长度异常生成残缺协议帧（Phase 231）
  ...
  - [x] 集群临时文件路径边界：保存 nodes.conf 前校验 `.tmp` 路径格式化，
    截断时放弃写入并保持脏状态（Phase 236）
  - [x] 服务控制面空指针保护：快照路径 setter 与集群启用入口拒绝 NULL/
  ...
  - [x] Tier 日志重放完整性：恢复记录按声明 body 长度精确读取，截断记录
    或长度算术溢出立即失败，避免建立无效索引（Phase 254）
  - [x] Tier 记录 ID/长度边界：重放和追加拒绝 `UINT64_MAX` ID 及超出 32 位
    记录长度，防止 next_id 回绕和索引定位截断（Phase 255）
  - [x] Tier 索引与偏移一致性：索引写入失败、文件偏移累加溢出和追加总长
    溢出均 fail-closed，避免持久化与内存索引分叉（Phase 256）
  - [x] 多数据库快照 API 边界：分片/多库序列化、保存和加载入口校验上下文、
    回调、路径及缓冲区，避免空参数导致状态恢复崩溃（Phase 251）
  ...
  - [x] 哈希表未初始化状态保护：CRUD、迭代和随机抽样检测空槽数组/零容量，
    避免损坏句柄触发掩码和指针解引用（Phase 266）
  - [x] 哈希表容量辅助边界：slot bytes/grow capacity 测试辅助接口拒绝 NULL
    输出指针，避免测试与诊断路径崩溃（Phase 267）
  - [x] 哈希表损坏状态防御：迭代、旧表查找和销毁路径校验槽数组/容量一致性，
    并防止异常别名状态触发 double-free（Phase 268）
  - [x] 哈希表输出确定性：`rh_set_ex2` 插入时清零旧值所有权输出，随机抽样
    失败时清零全部视图，避免调用方误释放或复用陈旧指针（Phase 269）
  - [x] 哈希表别名销毁安全：释放主/旧槽数组时识别指针别名，避免损坏句柄
  ...
  - [x] PAL 安全随机源输入边界：非零长度 NULL 缓冲区统一返回失败，零长度
    请求保持成功，避免跨平台系统随机 API 解引用无效地址（Phase 287）
  ...
  - [x] 集群节点表损坏状态防护：`nnodes` 负值或超过固定容量时，节点查找/
    新增、渲染、总线构帧和状态查询统一 fail-closed（Phase 296）
  - [x] 集群故障报告与状态恢复边界：`nreports` 异常时故障报告/仲裁拒绝遍历，
  ...
  - [x] 集群总线帧原子发布：v2 sender 扩展长度在节点表更新前完成校验，截断帧
  ...
  - [x] 快照加载关闭错误传播：单库与多库文件加载检查 `pal_file_close`，关闭
  ...
  - [x] Stream 字段视图校验：追加前拒绝非零长度 NULL 字段/值指针，避免损坏
  ...
  - [x] ZSET 迭代器空输入校验：迭代器创建、移动、比较和读取接口统一拒绝
  ...
  - [x] Quicklist 初始化空句柄校验：`ql_init(NULL)` 安全 no-op，保持生命周期
  ...
  - [x] ACL 空名称视图安全：内部大小写比较对零长度视图跳过 `memcmp`，删除
    入口拒绝 NULL/空用户名，避免空槽位被误删并保持合法用户名热路径不变
    （Phase 403）
  - [x] ACL 空频道视图安全：频道授权仅拒绝非零长度 NULL 频道，允许合法的
  ...
  - [x] 多线程复制空 key 恢复安全：全同步分片恢复对零长度 key 使用最小存储并跳过
  ...
  - [x] benchmark 维度策略上限：pipeline 限制为 1,048,576、client 限制为
    100,000，超过上限在任何内存分配前返回明确错误，避免合法整数参数导致
    极端资源申请（Phase 434）
  ...
  - [x] benchmark 异常路径清理：统一释放部分初始化连接、事件循环、arena
    ...
    （Phase 437）
  - [x] benchmark 端口回归确定性：使用已占用端口验证连接失败，并覆盖
    `65536` 上界错误，消除临时端口竞态（Phase 438）
  - [x] benchmark fd 状态失效：连接关闭后立即清零句柄，统一清理不会重复
  ...
  - [x] 历史实施计划状态收敛：根据现有代码、CTest 和提交记录，勾选 C 标准自适应
  ...
  - [x] Redis 7 兼容审计元数据校准：将文档中的 `CMD_TABLE` 顶层命令计数更新为
