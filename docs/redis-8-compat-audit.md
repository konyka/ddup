...
安全性增量（Phase 414）：`ARSCAN` 的 `LIMIT` 选项令牌经过统一视图校验，
malformed 非零长度 NULL 参数 fail-closed；合法数组扫描行为保持兼容。

安全性增量（Phase 415）：共享命令 token 比较器拒绝非零长度 NULL 视图，覆盖
数组扩展与 Cluster 等直接大小写比较路径，malformed 请求不再触发解引用。
...
可靠性增量（Phase 422）：多线程复制全同步分片恢复对零长度 key 分配最小存储并跳过
复制，避免 `malloc(0)` 平台差异导致合法空 key 无法恢复。
...
通用键移动边界补充复核：RENAME/RENAMENX、RPOPLPUSH、LMOVE 与 SMOVE 的同键
...
审计文档元数据补充复核（Phase 528）：Redis 7 历史报告中的 ddup `CMD_TABLE` 顶层
...
RedBus publish-frame construction now skips zero-length channel/message copies,
...
Listpack mutation helpers now fail closed for NULL listpacks and malformed
non-empty payload views, preventing lifecycle/control crashes while preserving
valid compact-entry encoding and replacement semantics.
