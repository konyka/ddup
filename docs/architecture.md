...
## Phase 474：MT 连接迁移 ACL 上下文保持

连接 rehome/adopt 除了转移数据库、输入输出缓冲和流水线状态，还会重新绑定
目标 worker 的 server-owned ACL 会话上下文。认证用户、启用状态、密码校验结果、
...
## Phase 498：ACL SETUSER 全类别规则

ACL 规则解析现在复用统一命令类别映射，支持 `+@`/`-@` 后的全部 Redis 8 类别，
包括 `array`、数据结构类别、访问属性及控制类别。类别展开只在用户配置变更时
遍历固定命令表；运行时授权继续读取预展开 bitset，不增加热路径分配或锁。

## Phase 500：ACL 扩展类别回归

新增回归覆盖 `+@array` 对 `ARGET` 的授权，以及 `+@ratelimit` 合法空类别不意外
放行普通命令。这样数据类型类别和未来限流类别的 fail-closed 边界均由测试锁定。

## Phase 501：ACL 类别规则负向回归

补充 `+@stream` 对 `XADD` 的正向授权以及未知 `+@unknown` 类别的拒绝测试，
覆盖数据类型规则、key pattern 校验和未知类别 fail-closed 行为。

## Phase 502：ACL 类别枚举数量回归

`ACL CAT` 无参数响应现在由测试锁定为 Redis 8.10.1 的 23 个默认类别，同时继续
检查代表性名称。该断言可在类别列表变更时及时暴露协议兼容性回退。

## Phase 503：ACL CAT SWAPDB keyspace 映射
...
## Phase 516：REPLCONF 选项值校验

`REPLCONF` 保留 Redis 复制握手所需的空参数、`GETACK *` 和通用未知选项兼容
...
## Phase 523：MEMORY STATS 字段收敛

`MEMORY STATS` 现在返回 Redis 8 统计对象的稳定核心字段：
