...
static void test_acl_deleted_recreated_session_is_denied_on_remote_route(void)
{
    mt_server *ms;
    pal_socket_t a, b;
    char key[32], req[256];
    ...
    roundtrip(b, "*6\r\n$3\r\nACL\r\n$7\r\nSETUSER\r\n$5\r\nalice\r\n$2\r\non\r\n$6\r\n>newpw\r\n$4\r\n+get\r\n", "+OK\r\n");
    snprintf(req, sizeof(req), "*2\r\n$3\r\nGET\r\n$%zu\r\n%s\r\n", strlen(key), key);
    roundtrip(a, req, "-NOAUTH Authentication required.\r\n");
    pal_close(a); pal_close(b);
    mt_server_stop(ms); mt_server_destroy(ms); pal_socket_cleanup();
}

static void test_acl_remote_denial_is_logged_on_home_worker(void)
{
    mt_server *ms;
    pal_socket_t a, b;
    char key[32], req[256], buf[2048];
    DD_CHECK_EQ_INT(0, pal_socket_init());
    pick_key_for_worker(1, 2, key, sizeof(key));
    ms = mt_server_create("127.0.0.1", 0, 2);
    DD_CHECK(ms != NULL); if (ms == NULL) { pal_socket_cleanup(); return; }
    mt_server_set_requirepass(ms, "rootpw");
    DD_CHECK_EQ_INT(0, mt_server_start(ms));
    a = connect_client(mt_server_port(ms)); b = connect_client(mt_server_port(ms));
    roundtrip(a, "*2\r\n$4\r\nAUTH\r\n$6\r\nrootpw\r\n", "+OK\r\n");
    roundtrip(a, "*6\r\n$3\r\nACL\r\n$7\r\nSETUSER\r\n$6\r\nreader\r\n$2\r\non\r\n$7\r\n>secret\r\n$4\r\n+get\r\n", "+OK\r\n");
    roundtrip(b, "*3\r\n$4\r\nAUTH\r\n$6\r\nreader\r\n$6\r\nsecret\r\n", "+OK\r\n");
    snprintf(req, sizeof(req), "*2\r\n$3\r\nSET\r\n$%zu\r\n%s\r\n", strlen(key), key);
    roundtrip(b, req, "-NOPERM this user has no permissions to run the command or access the key\r\n");
    roundtrip(b, "*2\r\n$4\r\nAUTH\r\n$6\r\nrootpw\r\n", "+OK\r\n");
    {
        size_t n = request_frame(b, "*2\r\n$3\r\nACL\r\n$3\r\nLOG\r\n", buf,
                                 sizeof(buf));
        DD_CHECK(n > 4 && buf[0] == '*' && buf[1] != '0');
    }
    (void)buf;
...
static void test_acl_nondefault_cannot_mutate_users_mt(void)
{
    mt_server *ms;
    pal_socket_t admin, reader;
    char key[32], req[192];
...
static void test_acl_revoke_is_enforced_at_mt_exec(void)
{
    mt_server *ms;
    pal_socket_t admin, user;
    char key[32], req[256];
    ...
    DD_CHECK_EQ_INT(0, mt_server_start(ms));
    admin = connect_client(mt_server_port(ms));
    user = connect_client(mt_server_port(ms));
    ...
    /* Revoke SET before EXEC; the queued write must not execute. */
    roundtrip(admin,
              "*4\r\n$3\r\nACL\r\n$7\r\nSETUSER\r\n$6\r\nreader\r\n"
              "$4\r\n-set\r\n",
              "+OK\r\n");
...
int main(void)
{
    ...
    DD_RUN(test_acl_nondefault_cannot_mutate_users_mt);
    DD_RUN(test_acl_revoke_is_enforced_at_mt_exec);
