...
static void test_handle_rejects_truncated_gossip_without_topology_mutation(void)
{
    db source, target;
    resp_buf frame, reply;
    uint32_t short_len;
    ...
    DD_CHECK_EQ_INT(0, cluster_bus_build_frame(&source, CLUSTER_MSG_PING,
                                               &frame));
    DD_CHECK(frame.len > 2);
    /* Remove the gossip-count trailer while preserving a matching header. */
    short_len = (uint32_t)(frame.len - 2);
    frame.data[4] = (char)(short_len & 0xffu);
    frame.data[5] = (char)((short_len >> 8) & 0xffu);
    frame.data[6] = (char)((short_len >> 16) & 0xffu);
    frame.data[7] = (char)((short_len >> 24) & 0xffu);
