#!/usr/bin/env python3
"""Run a reproducible ddup/Valkey matrix and emit JSON plus standalone HTML."""
import argparse
import html
import json
import platform
...
<p class='lede'>同一 RESP 压测客户端、同一 loopback 环境和固定请求矩阵下，对 ddup、ddup 多线程模式与 Valkey 9.0.4 进行吞吐和延迟对比。</p>
...
<script>const data=%s;const c=document.getElementById('chart'),x=c.getContext('2d');function draw(){const d=devicePixelRatio||1,w=c.clientWidth,h=c.clientHeight;c.width=w*d;c.height=h*d;x.scale(d,d);x.clearRect(0,0,w,h);const g={};data.forEach(v=>{const k=v.mode+' / P'+v.pipeline;(g[k]??={})[v.product]=v.rps||0});const ks=Object.keys(g),ps=[...new Set(data.map(v=>v.product))],m=Math.max(1,...data.map(v=>v.rps||0));ks.forEach((k,i)=>ps.forEach((p,j)=>{const v=g[k][p]||0,b=Math.max(10,(w-80)/(ks.length*ps.length)),xx=55+(i*ps.length+j)*b,yy=h-35-v/m*(h-70);x.fillStyle=['#d9573f','#26736b','#405a80'][j%%3];x.fillRect(xx,yy,b-3,h-35-yy)}));x.strokeStyle='#a9a294';x.beginPath();x.moveTo(45,10);x.lineTo(45,h-35);x.lineTo(w-10,h-35);x.stroke()}addEventListener('resize',draw);draw();</script></main></body></html>""" % (html.escape(payload["generated_at"]), "".join(cards), base["requests"], base["clients"], ", ".join(map(str, base["pipelines"])), base["value_size"], table, failures, html.escape(payload["generated_at"]), html.escape(payload["environment"]), html.escape(str(payload["cpu_count"])), data)
...
def main():
    ...
    products = [{"name": "ddup (1 worker)", "kind": "ddup", "server": ddup, "workers": 1},
                ...
                {"name": "Valkey 9.0.4", "kind": "valkey", "server": valkey, "workers": 1}]
